var News = (function () {
	...
	ScrollView.prototype = {
		...
		render : function() {
			try {
				var root = this.controller.getModel();
		...
		moveNews : function(direction) {
			this.moving = direction;
			var here = this;
			if (this.interval === null) {
				this.interval = window.setInterval(function() {
					here.callBackForMove(direction);					
				}, 15);
			}
		},
		callBackForMove : function (direction) {
			var journals = document.getElementById('journals');
			var curDistance = parseInt(journals.style.left);
			var status = 1;  // status1: increase, status-1: decrease
			var direction_status = 1;
			...
			if (!curDistance) {
				journals.style.left = (1*direction_status)+'%'; // 1% or -1%
			}

			if (curDistance >= 100 || curDistance <= -100) {
				this.stopNews();
				journals.style.left = '0%';
				return;
			}
			journals.style.left = (curDistance+(2*direction_status)) + '%';
	...
	NewsModel.prototype = {
		...
		setChild : function(child, position) {
			child.newsChain.parentModel = this;
			var newChildren = [];
			
			if (position === 'left') {
				newChildren.push(child);
				for (var i=0; i<2; i++) {
					newChildren.push(this.getChild(i));
				}
				this.newsChain.childModels = newChildren;
			}
		...
		getId : function(position) {
			...
			if (this.newsName !== 'rootNews') return null;
			var child = this.getChild(pos.indexOf(position));
			return child.newsId;
		},
		...
		addJournal : function(direction, journalData) {
			var tempNews = new NewsModel('root', 'tempNews', null);
			tempNews.init(journalData);

			var child = tempNews.getChild();
			this.setChild(child[0], direction);
			tempNews = null;
			this.notify();
		},
		notify : function() {
	...
	var ScrollNews = function(newsModel, scrollView) {
		...
		this.tempDataArr = null;
	};
	ScrollNews.prototype = {
		getModel : function() {
			return this.model;
		},
		moveTo : function(direction, journalData) {
			// rolling
			this.tempDataArr = [direction, journalData];
			this.view.moveNews(direction);			
		},
		isMoved : function () {
			// dataSet init
			var direction = this.tempDataArr[0];
			var journalData = this.tempDataArr[1];
			this.model.addJournal(direction, journalData);
			this.tempDataArr = null;
		},
	...
	var NewNews = function (elementId, journalData, templates, styles) {
		// model
		this.rootNews = new NewsModel('root', 'rootNews', null);
		this.rootNews.init(journalData);

		// view
		this.scrollView = new ScrollView(elementId, templates, styles);
	...
	NewNews.prototype = {
		moveTo : function(direction, journalData) {
			this.scrollNews.moveTo(direction, journalData);
		},
		getId : function (position) {
			return this.rootNews.getId(position);
		}
	}
