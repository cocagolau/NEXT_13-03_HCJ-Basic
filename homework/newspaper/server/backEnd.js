		...
		var methods = {
			nextKey : function (key) {
				if (typeof key === 'undefined')
					return;
				return keys[getIndex(key, 1)]; // getIndex가 null인 경우 처리 안함
			},
			previousKey : function (key) {
				if (typeof key === 'undefined')
					return;
				return keys[getIndex(key, -1)]; // getIndex가 null인 경우 처리 안함	
			},
			getValue : function (key) {
				return db.getValue('contents', key); // getIndex가 null인 경우 처리 안함
			},

			next : function (key) {
				var obj = {};
				var newKey = this.nextKey(key);
				var newValue = this.getValue(newKey);
				obj[newKey] = newValue;
				return obj;
			},
			previous : function (key) {
				var obj = {};
				var newKey = this.previousKey(key);
				var newValue = this.getValue(newKey);
				obj[newKey] = newValue;
				return obj;
			},
...
function DB () {
	this.template = {
		...
		article : '<article class="{ importance } btn"><h1><span>{ title }</span></h1><h2>{ strapline }</h2><address>{ author }</address></article>'
	};

	this.style = {
		// condition: forward, backward, random, custom
		journal : { form:'jourForm', journal:'journal', position:['forward', 'left', 'middle', 'right'] },
		article : { importance:['random', 'a1', 'a2', 'a3'] }
	};
